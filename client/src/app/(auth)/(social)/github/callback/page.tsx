...
const getAccessToken = async (props: any) => {
  const response = await fetch(
    `${process.env.SERVER_BASE_URL}/api/auth/github-callback?code=${props.searchParams.code}`,
    {
      cache: 'no-cache'
    }
  );
  ...
  if (response.status < 200 || response.status >= 399) {
    redirect(`/login?error=${decodeURIComponent(response.statusText)}`);
    // throw new Error(`HTTP Error : ${response.statusText}`);
  }
  const result = (await response.json()) as SocialLoginType;

  return result;
};

const SocialLoginCallbackPage: FC = async (props) => {
  const userInfo = await getAccessToken(props);

  if (userInfo?.statusCode === 301) {
    // redirect(
    //   `/register/social?type=${userInfo.data.userInfo.type}&id=${userInfo.data.userInfo.id}&avatar=${userInfo.data.userInfo.avatar}&userName=${userInfo.data.userInfo.userName}`
    // );
    return (
