...
const verifyEmail = async (token: string) => {
  const response = await fetch(
    `${process.env.SERVER_BASE_URL}/api/auth/verify-email?token=${token}`
  );
...
const VerifyEmailPage: FC<pageProps> = async ({ searchParams }) => {
  const emailVerify = (await verifyEmail(searchParams.token)) as
    | VerifyType
    | VerifyErrorType;

  if ('isEmailVerified' in emailVerify) {
